...
STREAMMODE1_ANY_OF_SCHEMAS = ["List[StreamMode]", "StreamMode"]


class StreamMode1(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = StreamMode1.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in StreamMode1 with anyOf schemas: List[StreamMode], StreamMode. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into StreamMode1 with anyOf schemas: List[StreamMode], StreamMode. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
